...
reorg<-function(x){
  ...
  Country<-Dat[which(Dat[,1]=="GEO"),2]  ## Country it was landed in (diff from flagship of vessel)
...
## EU data
setwd("C:/Users/t.tai/Documents/Price Database/International prices method & R scripts/Reported Price Data Aggregation/Extracted data/EU/Data/ExtractedNewPriceOnly16032016")
...
for(i in 1:length(foldernames)){
  ...
  wb<-read.csv(filenames,header=T,strip.white=T,stringsAsFactors=F,na.strings = c("",":"))
  wb1<-subset(wb,select=c(TIME,SPECIES,UNIT,GEO,Value))
  wb2<-na.omit(wb1)
  Out<-cast(wb2,TIME+SPECIES+GEO~UNIT)
...
dat<-merge(EU.Data,species,by.x="SPECIES",by.y="Species",all.x=T)

## extracting species not listed in SpeciesList for TaxonKeys
miss.spec<-unique(dat$SPECIES[which(is.na(dat$TaxonKey))])
miss.spec[miss.spec %in% species$Species]

setwd("C:/Users/t.tai/Documents/Price Database/International prices method & R scripts/Reported Price Data Aggregation/Extracted data")
write.csv(miss.spec,"missingspecies.csv",row.names=F)
